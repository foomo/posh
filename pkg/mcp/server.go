...
type (
	...
	listCommandsInput struct {
		// Path, if set, scopes the result to the single top-level command whose
		// full path matches exactly (e.g. "squadron"), instead of every command.
		Path string `json:"path,omitempty" jsonschema:"top-level command to scope the result to, e.g. \"squadron\"; omit for every command"`
		// Depth, if set, truncates each command's Subcommands beyond that many
		// levels, for a shallow overview of a large tree.
		Depth int `json:"depth,omitempty" jsonschema:"how many levels of subcommands to include; omit or 0 for the full tree"`
	}
	runCommandInput struct {
		Args []string `json:"args" jsonschema:"the posh command and its arguments, e.g. [\"cache\", \"clear\"]"`
	}
	runCommandOutput struct {
...
func (s *Server) Run(ctx context.Context) error {
	...
	sdkmcp.AddTool(server, &sdkmcp.Tool{
		...
		Description: "List this project's posh commands, the same catalog `posh agent catalog` prints. " +
			"Optionally scope to one top-level command via `path`, or limit subcommand depth via `depth`, " +
			"for projects whose full catalog is too large to return in one call.",
	}, s.listCommands)
...
func (s *Server) listCommands(ctx context.Context, req *sdkmcp.CallToolRequest, input listCommandsInput) (*sdkmcp.CallToolResult, any, error) {
	...
	catalog, err := ListCommands(ctx, plg, input.Path, input.Depth)
	if err != nil {
		return s.errorResult(err), nil, nil
	}
...
func (s *Server) errorResult(err error) *sdkmcp.CallToolResult {
